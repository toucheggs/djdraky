...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - tu canción **%s** suena ahora en %s!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = 'Sonando ahora en %s: **%s**' % (
        ...
        if not player.playlist.entries and not player.current_entry and self.config.auto_playlist:
            while self.autoplaylist:
                ...
                if not info:
                    ...
                    self.safe_print("[Info] Eliminando canciones no reproducibles desde autoplaylist: %s" % song_url)
                ...
                try:
                    ...
                    print("Error añadiendo canciones desde autoplaylist:", e)
                    continue

                break

            if not self.autoplaylist:
                print("[Warning] Sin canciones que sonar en la autoplaylist, desactivando.")
                self.config.auto_playlist = False
    ...
    async def update_now_playing(self, entry=None, is_paused=False):
        ...
        if self.user.bot:
            ...
            if activeplayers > 1:
                game = discord.Game(name="musica en %s servers" % activeplayers)
                entry = None
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Ahora sonando: **%s** añadida por **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = "Ahora sonando: **%s** %s\n" % (player.current_entry.title, prog_str)

            self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, np_text)
            await self._manual_delete_check(message)
        else:
            return Response(
                'No hay canciones en la cola! Coloca algo con {}play.'.format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Sonando ahora: **%s** añadida por **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append("Sonando ahora: **%s** %s\n" % (player.current_entry.title, prog_str))
